...
/**
 * An implementation of {@link PortTextViewBase} which represents the port of motors of LEGO MINDSTORMS EV3.
 */
public class Ev3MotorPortTextView extends PortTextViewBase {

    public Ev3MotorPortTextView(Context context, AttributeSet attrs) {
        super(context, attrs);
    }
    ...
    protected void savePortConnection(String port, String device) {
        ...
        if (c.getString(R.string.setting_portConnection_deviceMotorPortD).equals(port)) p.setOutputPortD(device);
    }
    ...
    protected void removePortConnection(String port) {
        ...
        if (c.getString(R.string.setting_portConnection_deviceMotorPortD).equals(port)) p.setOutputPortD(NONE);
    }
