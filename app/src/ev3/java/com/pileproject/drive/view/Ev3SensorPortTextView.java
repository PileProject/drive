...
/**
 * An implementation of {@link PortTextViewBase} which represents the port of sensors of LEGO MINDSTORMS EV3.
 */
public class Ev3SensorPortTextView extends PortTextViewBase {

    public Ev3SensorPortTextView(Context context, AttributeSet attrs) {
        super(context, attrs);
    }
    ...
    public static String getSensorName(Context context, String sensorType) {
        ...
        if (RANGEFINDER.equals(sensorType)) return context.getString(R.string.sensors_rangefinder);
        if (COLOR.equals(sensorType)) return context.getString(R.string.sensors_color);
        return "";
    }

    public static int getSensorColor(String sensorType) {
        if (TOUCH.equals(sensorType)) return Color.rgb(50, 142, 183);
        if (RANGEFINDER.equals(sensorType)) return Color.rgb(65, 163, 86);
        if (COLOR.equals(sensorType)) return Color.rgb(221, 86, 82);
        return Color.GRAY;
    }
